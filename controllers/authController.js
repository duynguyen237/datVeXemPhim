...
class AuthController {
    ...
    async login(req, res) {
        try {
            ...
            // 2. SO SÁNH MẬT KHẨU (Plain text vs Hashed string)
            // Lệnh này sẽ tự động lấy chuỗi băm trong DB và so sánh với mật khẩu khách nhập
            ...
            delete user.MAT_KHAU; // Xóa field mật khẩu trước khi trả về Frontend cho an toàn
            console.log(`🎬 Người dùng [${user.HO_TEN}] đã đăng nhập thành công!`);
            res.status(200).json({ success: true, message: "Đăng nhập thành công!", user: user });
